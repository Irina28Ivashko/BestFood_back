...
RecipeSchema.pre('save', async function (next) {
  ...
  for (const ingredient of this.ingredients) {
    ...
    if (ingredient.unit === 'шт') {
      // Если указано в штуках, используем unitWeight продукта, если он есть
      weight = product.unitWeight ? ingredient.amount * product.unitWeight : 0;
    } else if (product.measurements && product.measurements[ingredient.unit]) {
      // Если указано в специфических единицах измерения (стаканы, ложки)
      weight = product.measurements[ingredient.unit].gramsEquivalent * ingredient.amount;
    } else if (ingredient.unit === 'мл' && product.isLiquid) {
      // Для жидкостей, если указано в мл, используем прямое присвоение
      weight = ingredient.amount;
    } else if (ingredient.unit === 'гр') {
      // Если указано в граммах
      weight = ingredient.amount;
    }

    totalWeight += weight;

    // Пересчет питательных веществ на основе веса ингредиента
    totalCalories += (product.calories * weight) / 100;
    totalProteins += (product.proteins * weight) / 100;
    totalFats += (product.fats * weight) / 100;
    totalCarbohydrates += (product.carbohydrates * weight) / 100;
