...
RecipeSchema.pre('save', async function (next) {
  ...
  console.log(
    ...
    this.ingredients.map((ing) => ing.product.toString()),
  );

  for (const ingredient of this.ingredients) {
    console.log(`Looking up product with ID: ${ingredient.product}`);

    const product = await ProductModel.findById(ingredient.product);
    if (!product) {
      console.log(`Product not found for ID: ${ingredient.product}`);
      continue;
    }
    ...
    console.log(`Calculating nutritional values for product ${product.name}`);
    console.log(
      `Amount factor: ${amountFactor}, Calories: ${product.calories * amountFactor}, Proteins: ${
        product.proteins * amountFactor
      }, Fats: ${product.fats * amountFactor}, Carbohydrates: ${
        product.carbohydrates * amountFactor
      }`,
    );
  }
  ...
  console.log(
    `Total nutritional values for ${this.title}: Calories: ${totalCalories}, Proteins: ${totalProteins}, Fats: ${totalFats}, Carbs: ${totalCarbohydrates}`,
  );
