...
RecipeSchema.pre('save', async function (next) {
  ...
  for (const ingredient of this.ingredients) {
    ...
    let factor = weight / 100; // Пересчетный коэффициент для расчета на 100 грамм продукта
  ...
  if (totalWeight > 0) {
    this.totalCalories = (totalCalories / totalWeight) * 100;
    this.totalProteins = (totalProteins / totalWeight) * 100;
    this.totalFats = (totalFats / totalWeight) * 100;
    this.totalCarbohydrates = (totalCarbohydrates / totalWeight) * 100;
  }
