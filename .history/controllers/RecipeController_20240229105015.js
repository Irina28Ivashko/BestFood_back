...
export const searchRecipes = async (req, res) => {
  try {
    ...
    const recipesWithCommentsCount = await Promise.all(
      recipes.map(async (recipe) => {
        const recipes = await RecipeModel.find({ title: { $regex: regex } }).lean();
        // const commentsCount = await CommentModel.countDocuments({ recipe: recipe._id });
        return {
          ...recipe,
          commentsCount,
    ...
    console.log('Найденные рецепты:', recipesWithCommentsCount);
...
export const create = async (req, res) => {
  try {
    ...
    // Теперь получаем детальную информацию о рецепте, включая заполнение данных о продуктах
    const populatedRecipe = await RecipeModel.findById(recipe._id)
      .populate('ingredients.product', 'name calories proteins fats carbohydrates') // Указываем поля, которые хотим получить
      .exec();
