...
import { uploadAvatar, uploadImage } from './utils/fileFilter.js';
...
app.post('/upload/avatar', checkAuth, uploadAvatar.single('avatar'), (req, res) => {
  // Предполагается, что в req.file содержится информация о загруженном файле
  console.log(req.file);
  res.json({
    message: 'Аватар успешно загружен',
    url: `/uploads/avatars/${req.file.filename}`, // Путь должен соответствовать настройкам сохранения файлов в утилите
  });
});

// Загрузка изображений для рецептов или статей
app.post('/upload/image', checkAuth, uploadImage.single('image'), (req, res) => {
  res.json({
    message: 'Изображение успешно загружено',
    url: `/uploads/images/${req.file.filename}`, // Путь должен соответствовать настройкам сохранения файлов в утилите
  });
});
...
app.use(
  cors({
    origin: 'http://localhost:3000', // Указываем источник клиента
    credentials: true, // Разрешаем отправку cookies с запросами от клиента
  }),
);
