...
export const registerValidation = [
  ...
  body(
    'password',
    'Пароль должен быть минимум 6 символов и содержать только латинские буквы и цифры',
  )
    .isLength({ min: 6 })
    .matches(/^[a-zA-Z0-9]+$/, 'i')
    .withMessage('Пароль может содержать только латинские буквы и цифры.'),
  body('fullName', 'Укажите имя')
    .isLength({ min: 3 })
    .matches(/^[a-zA-Zа-яА-Я0-9-' ]+$/, 'i') // Добавлено регулярное выражение
    .withMessage('Имя может содержать только буквы, цифры, дефисы, апострофы и пробелы.'),
...
export const resetPasswordValidation = [
  body('email', 'Неверный формат почты').isEmail(),
  body('newPassword', 'Пароль должен быть минимум 6 символов')
    .isLength({ min: 6 })
    .matches(/^[a-zA-Z0-9]+$/, 'i')
    .withMessage('Пароль может содержать только латинские буквы и цифры.'),
